...
def classify_intent_node(state: AgentState) -> Dict[str, Any]:
    ...
    Supports typo-tolerant matching for schedule, calendar, events, and date queries.
    """
    ...
    # 2. Schedule Viewing intents (Typo-tolerant: "schedule", "scedule", "skedule", "calendar", "calender")
    elif any(view_kw in query_lower for view_kw in [
        "check", "view", "what is", "what's", "list", "show", "get", "for"
    ]):
        if any(kw in query_lower for kw in ["schedule", "scedule", "skedule", "calendar", "calender", "events", "meetings", "agenda"]):
    ...
    # 4. Event Creation / Scheduling intents ("add a birthday", "schedule a meeting", "book appointment")
    elif any(kw in query_lower for kw in ["birthday", "anniversary", "appointment", "meeting", "reminder", "event", "schedule", "scedule", "book", "slot"]):
        if any(action_kw in query_lower for action_kw in ["add", "create", "set", "put", "book", "mark"]):
            ...
            detected = IntentType.SCHEDULE_MEETING.value
        elif any(sched_kw in query_lower for sched_kw in ["schedule", "scedule", "skedule", "calendar", "calender"]):
            detected = IntentType.CHECK_SCHEDULE.value
        else:
            detected = IntentType.SCHEDULE_MEETING.value
